...
func NewCommand(ctx context.Context, opts *command.Options) *cobra.Command {
	return runner.SectionCommand(ctx, opts, "slm", "Show SLM (Snapshot Lifecycle Management) policies", Render)
}
...
// slmPolicies fetches the lifecycle policies and their stats, then merges
// them into one display-ready list per policy.
func slmPolicies(ctx context.Context, es *elasticsearch.Elasticsearch) ([]slmPolicy, error) {
	lifecycle, err := es.SlmLifecycle(ctx)
	if err != nil {
		return nil, err
	}

	statByPolicy, err := es.SlmStats(ctx)
	if err != nil {
		return nil, err
	}
	...
	for id, item := range lifecycle {
		...
		if stats, ok := statByPolicy[id]; ok {
			...
			policy.DeletionFailures = stats.SnapshotDeletionFailures
...
func formatSLMRetention(retention *elasticsearch.SlmRetention) string {
	if retention == nil {
		return "-"
	}
...
func formatSLMInvocation(invocation *elasticsearch.SlmInvocation) string {
	if invocation == nil {
		return "-"
	}
