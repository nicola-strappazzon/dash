...
func newNodesCommand(ctx context.Context, opts *command.Options) *cobra.Command {
	return sectionCommand(ctx, opts, "nodes", "Show cluster nodes")
}

func renderClusterNodes(ctx context.Context, es *dashboard.Elasticsearch) error {
	...
	for _, n := range nodes {
		tbl.Add(
			...
			n.IP,
			n.Master,
			common.ParseFloat(n.HeapPercent),
			common.ParseFloat(n.RAMPercent),
			common.ParseFloat(n.CPU),
			common.ParseFloat(n.DiskUsedPercent),
			common.ParseUptime(n.Uptime),
		)
	}
