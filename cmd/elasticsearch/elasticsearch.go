...
func NewCommand(ctx context.Context, opts *command.Options) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "elasticsearch [all|health|nodes|indices|shards|allocation|settings|recovery|snapshots|slm|thread...]",
	...
	cmd.AddCommand(
		...
		allocation.NewCommand(ctx, opts),
		settings.NewCommand(ctx, opts),
		recovery.NewCommand(ctx, opts),
		snapshots.NewCommand(ctx, opts),
		slm.NewCommand(ctx, opts),
		thread.NewCommand(ctx, opts),
	)
