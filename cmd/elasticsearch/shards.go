...
func newShardsCommand(ctx context.Context, opts *command.Options) *cobra.Command {
	return sectionCommand(ctx, opts, "shards", "Show shards")
}

func renderShards(ctx context.Context, es *dashboard.Elasticsearch) error {
	...
	for _, s := range shards {
		tbl.Add(
			s.Index,
			common.ParseInt(s.Shard),
			strings.ToUpper(s.Prirep),
			s.State,
			common.ParseInt(s.Docs),
			common.ParseFloat(s.Store),
			s.IP,
			s.Node,
		)
	}
